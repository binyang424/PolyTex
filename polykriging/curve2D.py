...
def curveKrig(dataset, name_drift, name_cov, nuggetEffect=0):
    ...
    for i in np.arange(len_b):
        for j in np.arange(len_b):
            if i <= j:
                # The Euclidean distance between two points in 1 dimensions.
                h = abs(dataset[:, 0][i] - dataset[:, 0][j])
    ...
    doc_krig = a_drift * func_drift(x, adef)

    for i in np.arange(len_b):
        bi_cov = ((func_cov((x - dataset[:, 0][i]) ** 2 )) ** (1 / 2)) * vector_ba[i, 0]
